...
`;
  ...
  // ---- Compile & Run: tries multiple free C compilers in order ----------
  // If the first one fails (down, rate-limited, network-blocked), the next
  // is tried automatically. No API key required for any of these.
  const RUNNERS = [
    {
      name: "Codex (Jaagrav)",
      ...
      run: async (code, stdin) => {
        const resp = await fetch("https://api.codex.jaagrav.in/", {
          ...
          body: JSON.stringify({ code, language: "c", input: stdin || "" })
        });
        if (!resp.ok) throw new Error("HTTP " + resp.status);
        const data = await resp.json();
        let out = "";
        if (data.error && String(data.error).trim()) out += data.error + "\n";
        if (data.output) out += data.output;
        const failed = !!(data.error && String(data.error).trim());
        return { output: out.trim() || "(no output)", failed };
      }
    },
    {
      ...
      run: async (code, stdin) => {
        ...
        const run     = data.run     || {};
        let out = "";
        if (compile.stderr && compile.stderr.trim()) out += "[Compile]\n" + compile.stderr + "\n";
    ...
    },
    {
      ...
      run: async (code, stdin) => {
        const resp = await fetch("https://wandbox.org/api/compile.json", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ compiler: "gcc-head-c", code, stdin: stdin || "" })
        });
        if (!resp.ok) throw new Error("HTTP " + resp.status);
        const data = await resp.json();
        let out = "";
        if (data.compiler_error && data.compiler_error.trim()) out += "[Compile]\n" + data.compiler_error + "\n";
        if (data.program_output) out += data.program_output;
        if (data.program_error && data.program_error.trim()) out += "\n[stderr]\n" + data.program_error;
        const failed = !!(data.compiler_error && data.compiler_error.trim());
        return { output: out.trim() || "(no output)", failed };
      }
    }
  ];
  ...
  async function runCode() {
    ...
    for (const runner of RUNNERS) {
      try {
        ...
        outputEl.className = result.failed ? "error" : "success";
        outputEl.textContent = result.output;
        return; // success — stop trying
      } catch (e) {
        failures.push(`${runner.name}: ${e.message}`);
        // try the next runner
      }
    }
    ...
    outputEl.textContent =
      "Could not reach any in-browser C compiler. Your network may be blocking them,\n" +
      "or all three services are temporarily down.\n\n" +
      "Workaround that always works:\n" +
      "  Click \"Open in OnlineGDB ↗\" above — your code is copied to the clipboard,\n" +
      "  OnlineGDB opens in a new tab, paste with Ctrl+V and click Run.\n\n" +
      "Technical details:\n  " + failures.join("\n  ");
  }
